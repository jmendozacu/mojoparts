...
class CommerceExtensions_Orderimportexport_Model_Convert_Parser_Exportorders
    extends Mage_Eav_Model_Convert_Parser_Abstract
{
    ...
    public function unparse()
    {
				...
				if($this->getVar('date_from') != "" && $this->getVar('date_to') != "" && $this->getVar('filter_by_order_status') != "" && $this->getVar('filter_by_store_id') != "") {
					...
					$orders = Mage::getModel('sales/order')->getCollection()
									...
									->load();
				} else if($this->getVar('date_from') != "" && $this->getVar('date_to') != "" && $this->getVar('filter_by_order_status') != "" ) {
					...
					$orders = Mage::getModel('sales/order')->getCollection()
									...
									->addAttributeToFilter ( 'created_at' , array( "from" => $date_from, "to" => $date_to, "datetime" => true ))
									->load();
				} else if($this->getVar('filter_by_order_status') != "" && $this->getVar('filter_by_store_id') != "") {
					$orders = Mage::getModel('sales/order')->getCollection()
									->addAttributeToSelect('*')
									->addAttributeToFilter ( 'store_id' , $this->getVar('filter_by_store_id'))
									->addFieldToFilter ( 'status' , array( "eq" => $this->getVar('filter_by_order_status') ))
						...
						foreach ($items as $itemId => $item)
						{
							 ...
							 */
							 ...
							 if ($item->getProductType() == "bundle") {
								 ...
								 $productbundleItem = Mage::getModel('catalog/product')->load($order_prodID); 
								 #$bundleskuforexport =  $productbundleItem->getSku();
								 $bundleskuforexport =  trim($productbundleItem->getSku(), '() ');
								 ...
								 #print_r($bundle_json_array['bundle_options']);
								 $optionModel = Mage::getModel('bundle/option')->getResourceCollection()->setProductIdFilter($order_prodID);
								...
								foreach($optionModel as $eachOption) {
									...
									#print_r($ordered_bundle_options);
									...
									foreach($selectionModel as $eachselectionOption) {
										if($eachselectionOption->getData('option_id') == $eachOption->getData('option_id')) {
											#print_r($eachselectionOption->getData());
								   $productbundleselectItem = Mage::getModel('catalog/product')->load($eachselectionOption->getData('product_id')); 											
								   			if($productbundleselectItem->getName() == $ordered_bundle_options['value'][0]['title']) {
											   #echo "NAME: " . $productbundleselectItem->getName() . "<br/>";
								   		       #echo "NAME1: " . $ordered_bundle_options['value'][0]['title'] . "<br/>";
											   $single_bundle_sku = str_replace($productbundleItem->getSku(), "", $item->getSku());
											   if($eachselectionOption->getData('sku') == $single_bundle_sku) {
							   					if($this->getVar('export_product_pricing') == "true") {
												$finalbundleoptions .= $eachselectionOption->getData('sku') . "~" .$ordered_bundle_options['value'][0]['qty']. "^" . $ordered_bundle_options['value'][0]['price']. "^" . $ordered_bundle_options['value'][0]['title']. "^";
												} else {
												$finalbundleoptions .= $eachselectionOption->getData('sku') . "~" .$ordered_bundle_options['value'][0]['qty']. "^";
												}
