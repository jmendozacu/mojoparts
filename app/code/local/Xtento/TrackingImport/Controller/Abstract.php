...
class Xtento_TrackingImport_Controller_Abstract extends Mage_Adminhtml_Controller_Action
{
    /**
    ...
    protected function _prepareFileDownload($fileArray)
    {
        if (count($fileArray) > 1) {
            ...
            if (!@class_exists('ZipArchive') && !function_exists('gzopen')) {
                $this->_getSession()->addError(Mage::helper('xtento_trackingimport')->__('PHP ZIP extension not found. Please download files manually from the server, or install the ZIP extension, or import just one file with each profile.'));
            ...
            if (@class_exists('ZipArchive')) {
                ...
                if ($zipArchive->open($zipFile, ZIPARCHIVE::CREATE) !== TRUE) {
                    $this->_getSession()->addError(Mage::helper('xtento_trackingimport')->__('Could not open file ' . $zipFile . '. ZIP creation failed.'));
                    return $this->_redirectReferer();
                }
                ...
                require_once(Mage::getModuleDir('', 'Xtento_TrackingImport') . DS . 'lib' . DS . 'PclZip.php');
                $zipFile = tempnam(sys_get_temp_dir(), 'zip');
            ...
            if (!$zipFile) {
                $this->_getSession()->addError(Mage::helper('xtento_trackingimport')->__('ZIP file couldn\'t be created.'));
                return $this->_redirectReferer();
            }
    ...
    private function _healthCheck()
    {
        ...
        if (!Mage::helper('xtento_trackingimport')->isModuleProperlyInstalled()) {
            if ($this->getRequest()->getActionName() !== 'installation') {
                $this->getResponse()->setRedirect($this->getUrl('*/trackingimport_index/installation'));
            ...
            if ($this->getRequest()->getActionName() == 'installation') {
                $this->getResponse()->setRedirect($this->getUrl('*/trackingimport_profile/index'));
                $this->getResponse()->sendResponse();
                $this->getRequest()->setDispatched(true);
                return $this;
            }
        ...
        // Check module status
        if (!Mage::getBlockSingleton('xtento_trackingimport/adminhtml_widget_menu')->isEnabled() || !Mage::helper('xtento_trackingimport')->getModuleEnabled()) {
            if ($this->getRequest()->getActionName() !== 'disabled') {
                $this->getResponse()->setRedirect($this->getUrl('*/trackingimport_index/disabled'));
                $this->getResponse()->sendResponse();
                $this->getRequest()->setDispatched(true);
                return $this;
            }
        ...
        if ($this->getRequest()->getActionName() !== 'redirect') {
            ...
            if (Xtento_TrackingImport_Helper_Data::EDITION !== 'CE' && Xtento_TrackingImport_Helper_Data::EDITION !== '') {
            ...
            if (!Mage::getStoreConfig('trackingimport/general/disable_cron_warning')) {
                    ...
                    if (!Mage::helper('xtcore/utils')->isCronRunning()) {
                        if ((time() - Mage::helper('xtcore/data')->getInstallationDate()) > (60 * 30)) { // Module was not installed within the last 30 minutes
                            if (Mage::helper('xtcore/utils')->getLastCronExecution() == '') {
                                $this->addWarning(Mage::helper('xtento_trackingimport')->__('Cronjob status: Cron.php doesn\'t seem to be set up at all. Cron did not execute within the last 15 minutes. Please make sure to set up the cronjob as explained <a href="http://support.xtento.com/wiki/Setting_up_the_Magento_cronjob" target="_blank">here</a> and check the cron status 15 minutes after setting up the cronjob properly again.'));
                            } else {
                                $this->addWarning(Mage::helper('xtento_trackingimport')->__('Cronjob status: Cron.php doesn\'t seem to be set up properly. Cron did not execute within the last 15 minutes. Please make sure to set up the cronjob as explained <a href="http://support.xtento.com/wiki/Setting_up_the_Magento_cronjob" target="_blank">here</a> and check the cron status 15 minutes after setting up the cronjob properly again.'));
                            }
                        } else {
