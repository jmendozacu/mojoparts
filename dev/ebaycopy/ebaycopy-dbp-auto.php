...
while ($input_row = mysqli_fetch_array($input_result)) { 
	...
	$processThisSku = TRUE;
	$restricted = FALSE;
	$reviewCode = "ebaycopy-".date("Y.m.d");
	$side = "";
	$surfaceFinish = ""; 
	$upc = "Does Not Apply";
	$vendor = "PFG";
	...
	if( ! $curl_result = curl_exec($ch))
    {
		...
		$vendorSkuStartPattern = "<strong style=\"font-size:12px; color:#006600;\">Description / Notes:</strong><span style=\"margin-left:10px; color:#fff;\">";
		...
		$vendorSku = substr($listingHtml, $vendorSkuStartPos+120, $vendorSkuEndPos-$vendorSkuStartPos-120);
		if (substr($vendorSku, -1) == "Q") { 
			$capa = TRUE; 
		}
//echo $vendorSku.PHP_EOL;
		if (substr($vendorSku,0,3) == "PZN") { 
