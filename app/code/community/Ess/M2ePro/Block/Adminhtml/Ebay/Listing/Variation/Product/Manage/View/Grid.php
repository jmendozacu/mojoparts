...
HTML;
    ...
    {
        $deletedMagentoVariationsLink = '';

        $deletedVariations = $this->getListingProduct()->getSetting(
            'additional_data', 'variations_that_can_not_be_deleted'
        );

        if ($this->canDisplayContainer() && !empty($deletedVariations)) {
            $showDeletedMagentoVariationsText = Mage::helper('M2ePro')->__('Show Old Inactive Variations');
            $hideDeletedMagentoVariationsText = Mage::helper('M2ePro')->__('Hide Old Inactive Variations');

            $deletedMagentoVariationsGrid = $this->getLayout()
                ->createBlock('M2ePro/adminhtml_ebay_listing_variation_product_manage_view_deletedVariations_Grid');
            $deletedMagentoVariationsGrid->setListingProduct($this->getListingProduct());


            $deletedMagentoVariationsLink = <<<HTML
<div style="text-align: right;">
    <a href="javascript:void(0);" id="deleted_magento_variations_show" 
        onclick="VariationsGridHandlerObj.showDeletedMagentoVariations();">
        {$showDeletedMagentoVariationsText}
    </a>
    <a href="javascript:void(0);" id="deleted_magento_variations_hide" 
        onclick="VariationsGridHandlerObj.hideDeletedMagentoVariations();" 
        style="display:none">
        {$hideDeletedMagentoVariationsText}
    </a>
</div>
<div id="deleted_magento_variations_grid" style="padding-top: 10px; display:none">
    {$deletedMagentoVariationsGrid->toHtml()}
</div>
HTML;
