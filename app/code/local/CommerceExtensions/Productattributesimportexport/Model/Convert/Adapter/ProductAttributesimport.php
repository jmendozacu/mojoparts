						 ...
						 foreach ($pipedelimiteddata as $datalabel) {
							 $pos = strpos($datalabel, ":");
							   if ($pos !== false) {
									...
									if($this->getBatchParams('map_store_ids') != "") {
										$mapped_store_values = explode(",",$this->getBatchParams('map_store_ids'));
										foreach ($mapped_store_values as $mapped_values) {
											$mapped_value = explode("=",$mapped_values);
											if($pipedelimiteddata1[0] == $mapped_value[0]) {
												$imported_store_id = $mapped_value[1];
											} else {
												$imported_store_id = $pipedelimiteddata1[0];
											}
										}
									} else {
										$imported_store_id = $pipedelimiteddata1[0];
									...
									//SITE 2 SITE TRANSFER ADDED ONLY
									$frontEndLabel[$imported_store_id] = $pipedelimiteddata1[1];
								} else {
							  ...
							  #$data['option']['value']['option_'.$attributevaluecounter.''] = array(0 => $actualattributeoptiondata[1], 2 => "test2");
								if(isset($actualattributeoptiondata[1])) {
									...
									if($this->getBatchParams('map_store_ids') != "") {
										...
										foreach ($mapped_store_values as $mapped_values) {
											$mapped_value = explode("=",$mapped_values);
											if($actualattributeoptiondata[0] == $mapped_value[0]) {
												$imported_store_id = $mapped_value[1];
											} else {
												$imported_store_id = $actualattributeoptiondata[0];
											}
										}
									} else {
										$imported_store_id = $actualattributeoptiondata[0];
									...
									//SITE 2 SITE TRANSFER ADDED ONLY
									#echo "STORE INSERT: " . $imported_store_id;
									
									$optionDataValues[$imported_store_id] = $actualattributeoptiondata[1];
									  ...
									  $thisotherdatacommadelimited = explode($attribute_options_values_delimiter,$options_data_update);
										foreach ($thisotherdatacommadelimited as $options_data1) {
											...
											if($this->getBatchParams('map_store_ids') != "") {
												...
												foreach ($mapped_store_values as $mapped_values) {
													$mapped_value = explode("=",$mapped_values);
													if($actualattributeoptiondata_update[0] == $mapped_value[0]) {
														$imported_store_id = $mapped_value[1];
													} else {
														$imported_store_id = $actualattributeoptiondata_update[0];
													}
												}
											} else {
												$imported_store_id = $actualattributeoptiondata_update[0];
											...
											if(!$this->attributeValueExists($importData["attribute_name"], $actualattributeoptiondata_update[1], $imported_store_id))
        									{
											#echo "HERE" . $actualattributeoptiondata_update[1];
									
											if($actualattributeoptiondata_update[1] !="") {
												#echo "STORE UPDATE: " . $imported_store_id;
												$result['value']['option_'.$attributeupdatevaluecounter.''][$imported_store_id] = $actualattributeoptiondata_update[1];		
