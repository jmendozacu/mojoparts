...
class Ess_M2ePro_Model_Mysql4_Listing_Product_Instruction extends Ess_M2ePro_Model_Mysql4_Abstract
{
    ...
    public function add(array $instructionsData)
    {
        ...
        foreach ($instructionsData as $index => &$instructionData) {
            ...
            if (is_null($listingProduct)) {
                $missingListingProducts[] = $instructionData['listing_product_id'];
        ...
        if (!empty($missingListingProducts)) {
            Mage::helper('M2ePro/Module_Logger')->process($missingListingProducts, 'Missing Listing products', false);
        }
    ...
    public function remove(array $instructionsIds)
    {
        ...
        $this->_getWriteAdapter()->delete($this->getMainTable(), array('id IN (?)' => $instructionsIds));
    }
