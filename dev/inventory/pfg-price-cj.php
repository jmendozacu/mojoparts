			...
			if ($listingStatus == 3 || $listingStatus == 4) { 
				if ($daysSinceLastSale < 365) {
					if ($calcedMinPrice > $currentPrice) { // it's priced below floor, so even though it hasn't sold in 60d, it has sold >60d, let's try to keep the price as low as possible 
						$newPrice = $calcedMinPrice;
						$reasonCode = 5;
						$reason = "5) The PFG costs are the same, the listing is manually stopped or finished, the most recent sale has been within 1Y although not within 60d (".$daysSinceLastSale." days), but it's currently priced (".$currentPrice.") below floor (".$calcedMinPrice."). So we need to at least set the new price = the minimum price.";
					}
					else { // this has had sales in the last year, but it's dead, so it's a candidate to randomize the price
						$randomIncrease = rand(100,200);
						$newPrice = $currentPrice + $randomIncrease/100;
						$reasonCode = 6;
						$reason = "6) The PFG costs are the same, the listing is manually stopped or finished due to no recent sales, so randomly adjust the price to cause it to refresh.";
					}
				}
					...
					if ($calcedMinPrice > $currentPrice) {
						...
						$newPrice = $currentPrice + $randomIncrease/100;
						$reasonCode = 17;
						$reason = "17) The PFG costs are the same, the listing is manually stopped or finished due to no sales in over a year, so randomly adjust the price to cause it to refresh.";
					}
						...
						$reasonCode = 7;
						$reason = "7) The PFG costs are the same, the listing is manually stopped or finished, it hasn't sold within 1Y (".$daysSinceLastSale." days), and it's currently priced (".$currentPrice.") above floor (".$calcedMinPrice."). So let's try reducing the profit% (".$currentProfitPct.") by 3%.";
					}
				}
			}
				...
				if ($calcedMinPrice > $currentPrice) { // it is underpriced, need to move up to floor
					...
					if ($currentPrice >= 99999.99) { 
						$newPrice = $tempPrice15; 
						$reasonCode = 14;
						$reason = "14) The listing NOT manually stopped, and it's currently priced with the dummy starting price of $99999.99, so set it to the 15% profit price.";
					} 
					else {
