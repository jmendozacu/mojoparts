...
extends Mage_Catalog_Model_Convert_Adapter_Product
{
	...
	*/
	public function saveRow( array $importData )
	{
		...
		if($this->getBatchParams('product_map_store_codes') != "") {
			$mapped_store_values = explode(",",$this->getBatchParams('product_map_store_codes'));
			foreach ($mapped_store_values as $mapped_values) {
				$mapped_value = explode("=",$mapped_values);
				if($importData['store'] == $mapped_value[0]) {
					$importData['store'] = $mapped_value[1];
				}
			}
		}
		
		if($this->getBatchParams('product_map_website_codes') != "") {
			$mapped_store_values = explode(",",$this->getBatchParams('product_map_website_codes'));
			foreach ($mapped_store_values as $mapped_values) {
				$mapped_value = explode("=",$mapped_values);
				if($importData['websites'] == $mapped_value[0]) {
					$importData['websites'] = $mapped_value[1];
				}
			}
		}
		
		if($this->getBatchParams('product_map_store_ids') != "") {
			$mapped_store_values = explode(",",$this->getBatchParams('product_map_store_ids'));
			foreach ($mapped_store_values as $mapped_values) {
				$mapped_value = explode("=",$mapped_values);
		...
		if ($importData['type'] == 'downloadable' && $importData['downloadable_options'] != "") {
			if ($new) {
				...
				foreach ($commadelimiteddata as $data) {
					...
					if ($importData['type'] == 'downloadable' && $importData['downloadable_sample_options'] != "") {
