...
class Ess_M2ePro_Model_Servicing_Task_Statistic extends Ess_M2ePro_Model_Servicing_Task
{
    ...
    private function appendExtensionSettingsInfo(&$data)
    {
        ...
        $data['settings']['track_direct'] = $config->getGroupValue(
            '/cron/task/listing/product/inspect_direct_changes','mode'
        );
    ...
    private function appendExtensionListingsProductsInfo(&$data)
    {
        ...
        $queryStmt = $connRead->select()
              ->from(Mage::helper('M2ePro/Module_Database_Structure')->getTableNameWithPrefix('m2epro_listing'),
                     array(
                         'component'      => 'component_mode',
                         'marketplace_id' => 'marketplace_id',
                         'account_id'     => 'account_id',
        ...
        foreach ($availableComponents as $nick) {
            ...
            foreach ($productTypes as $type) {
                ...
                if ($nick === Ess_M2ePro_Helper_Component_Amazon::NICK ) {
                    $select->joinLeft(
                ...
                $data['listings_products'][$nick]['products']['type'][$type] = array(
                    'amount' => $connRead->fetchOne($select)
                );
            }
        }
