...
# so that the listing can be refreshed.  Generate a csv containing a list of the
...
# catalog listings.
#
# A listing is conisdered stale if it hasn't sold within the current listing
...
def refresh_listing(row, cursor):
...
# analyzed to be refreshed.  The goal is to eventually distribute the listings
# evenly across 30 days without unnecessarily ending listings prematurely.  So
# we only analyze 1/30th of the listings ending the soonest, and if they haven't
# performed well then we'll refresh them.
pfg_goal = 0
other_goal = 0
        ...
        with open("data/active-listings.csv", "r") as input_file:
            ...
            for csv_row in map(Listing._make, reader):
                ...
                if (csv_linecode == PFG and pfg_count < pfg_goal) or\
                    ...
                    # Calculate the next end date based on the calculated most
                    # recent start date.
                    ...
                    if end_date == target_end_date:
                        if not is_recently_sold(row.last_sale, row.sold_qty) \
                            ...
                            refresh_listing(csv_row, db_cursor)
...
db_connection.close()
print("Script complete!  Goal # of listings processed:")
