...
$stmt = $connection->query(<<<SQL
    ...
    AND `elp`.`online_start_price` > 0 AND `elp`.`online_is_variation` IS NULL AND `elp`.`online_is_auction_type` IS NULL
SQL
);
...
$installer->getTableModifier('ebay_template_description')
          ->dropColumn('watermark_mode')
          ->dropColumn('watermark_image')
          ->dropColumn('watermark_settings');
...
$scheduledActionsTable = $installer->getTablesObject()->getFullName('listing_product_scheduled_action');
$query = $connection->select()
    ->from($scheduledActionsTable, array('id', 'additional_data'))
    ->where("additional_data LIKE '%\"shipping\"%'")
    ->query();

while ($row = $query->fetch()) {

    $additionalData = (array)json_decode($row['additional_data'], true);
    if (!isset($additionalData['configurator']['allowed_data_types'])) {
        continue;
    }

    $index = array_search('shipping', $additionalData['configurator']['allowed_data_types'], true);
    unset($additionalData['configurator']['allowed_data_types'][$index]);
