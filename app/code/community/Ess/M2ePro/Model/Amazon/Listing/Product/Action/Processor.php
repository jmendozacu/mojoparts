...
class Ess_M2ePro_Model_Amazon_Listing_Product_Action_Processor
{
    ...
    public function process()
    {
        ...
        foreach ($merchantIds as $merchantId) {
            ...
            $this->fillFeedsPacks(
                ...
                true, $availableRequestsCount
            );

            $filledListingsProductsIds = $this->getFilledListingsProductsIds($feedsPacks);

            $this->fillFeedsPacks(
                $feedsPacks,
                $this->getScheduledActionsDataStatement($merchantId, false, $filledListingsProductsIds),
                false
            );

            $actionsDataForProcessing = $this->prepareAccountsActions($feedsPacks);

            foreach ($actionsDataForProcessing as $accountId => $actionsData) {
                foreach ($actionsData as $actionType => $listingsProductsData) {
                    if (empty($listingsProductsData)) {
                        continue;
                    }

                    $this->initProcessingActions($actionType, $listingsProductsData);
                                    ...
                                    $availableRequestsCount = NULL)
    {
        while ($scheduledActionData = $scheduledActionsDataStatement->fetch()) {
            ...
            $accountId = $scheduledActionData['account_id'];

            if (!$canCreateNewPacks && !$this->areFeedTypesAllowed($feedTypes, $feedsPacks, $accountId)) {
                continue;
            }
            ...
            foreach ($feedTypes as $feedType) {
                if ($this->canAddToLastExistedPack($feedsPacks, $feedType, $accountId)) {
                    $canBeAdded = true;
                    continue;
                }
     ...
     * @return bool
     */
    private function areFeedTypesAllowed(array $feedTypes, array $feedsPacks, $accountId)
    {
        foreach ($feedTypes as $feedType) {
            $existingFeeds = count($feedsPacks[$feedType][$accountId]);
    ...
    private function getFilledListingsProductsIds(array $feedsPacks)
    {
            ...
            foreach ($feedPacks as $accountId => $accountPacks) {
                foreach ($accountPacks as $accountPack) {
                    foreach ($accountPack as $listingProductData) {
                        $listingsProductsIds[] = $listingProductData['listing_product_id'];
                    }
                }
            }
        }

        return array_unique($listingsProductsIds);
    }
     ...
     * @return string
     */
    private function initProcessingActions($actionType, array $listingsProductsData)
    {
    ...
    private function prepareProcessingActions()
    {
        ...
        /** @var  Ess_M2ePro_Model_Mysql4_Amazon_Listing_Product_Action_Processing_Collection $processingActionCollection */
        $processingActionCollection = Mage::getResourceModel(
            'M2ePro/Amazon_Listing_Product_Action_Processing_Collection'
        );
        $processingActionCollection->addFieldToFilter('is_prepared', 0);
        $processingActionCollection->getSelect()->limit($processingActionPreparationLimit);
        $processingActionCollection->getSelect()->order('id ASC');
        ...
        $listingsProductsIds = $processingActionCollection->getColumnValues('listing_product_id');

        /** @var Ess_M2ePro_Model_Mysql4_Listing_Product_Collection $listingsProductsCollection */
        $listingsProductsCollection = Mage::helper('M2ePro/Component_Amazon')->getCollection('Listing_Product');
        $listingsProductsCollection->addFieldToFilter('id', array('in' => $listingsProductsIds));

        $processingIds = $processingActionCollection->getColumnValues('processing_id');
    ...
    private function processGroupedProcessingActions(array $processingActions)
    {
        ...
        $actionType = reset($processingActions)->getType();
