...
class CommerceExtensions_Categoriesimportexport_Model_Convert_Adapter_Categoryimport extends Mage_Eav_Model_Convert_Adapter_Entity
{
    ...
    public function saveRow(array $importData)
    {
		...
		if($this->getBatchParams('map_store_codes') != "") {
			$mapped_store_values = explode(",",$this->getBatchParams('map_store_codes'));
		...
		if($this->getBatchParams('map_root_ids') != "") {
			$mapped_store_values = explode(",",$this->getBatchParams('map_root_ids'));
			foreach ($mapped_store_values as $mapped_values) {
				$mapped_value = explode("=",$mapped_values);
				if($importData['root_id'] == $mapped_value[0]) {
					$importData['root_id'] = $mapped_value[1];
				}
			}
		}
        ...
        if (empty($this->_categoryCache[$store->getId()])) {
			...
			*/
			$collection = Mage::getModel('catalog/category')->getCollection()
                ->addAttributeToSelect('name')
                ->addAttributeToSelect('path',array('like'=>$rootPath."/%"))
                ->setStoreId($store);
        ...
        if (!empty($cache[$importData['categories']])) {
            #return true; //maybe needs to be commented out
        }
