...
/**
	...
	public function exportProductData($product, $export_multi_store, $i, $websiteCode="")
    {
				...
				if($this->getVar('export_category_paths') == "true") {
					...
					/* PRODUCT CATEGORIES EXPORT END */
					 ...
					 /* ADDITIONAL CATEGORY ID EXPORT FOR 1.4 ONLY [END] */
						...
						if (count($galleryImagesModel) > 0) {
								foreach ($galleryImagesModel as $_image) {
									$finalgallerylabelimages .= $_image->getLabel() . ";";
									//$finalgalleryimages .= $_image['file'] . ", ";
									//$finalgallerylabels .= $_image['label'] . ", ";
								}
						}
						...
						$_linkSampleCollection = Mage::getModel('downloadable/sample')->getCollection()
						 ...
						 foreach ($_linkSampleCollection as $sample_link) {
							...
							if($sample_link->getSampleUrl() !="") {
								$finaldownloabledsampleproductoptions .= $link->getTitle() . "," . $link->getLinkType() . "," . $link->getSampleUrl() . "|";
							} else if($sample_link->getSampleFile() !="") {
								$finaldownloabledsampleproductoptions .= $link->getTitle() . "," . $link->getLinkType() . "," . $link->getSampleFile() . "|";
							}
						 }
    ...
    public function unparse()
    {
        ...
        $entityIdData = $this->getData();
		$entityIds = Mage::getModel('catalog/product')->getCollection()
									->addAttributeToSelect('*')
									->addAttributeToFilter('entity_id', array('in' => $entityIdData))
        ...
        foreach ($entityIds as $i => $entityId) {
			...
			if ($overallcount < $recordlimitend && $overallcount >= $recordlimitstart) {
				...
				if($this->getVar('export_multi_store') == "true") {	
					$allproduct = $this->getProductModel()->setData(array())->load($entityId->getId());
					...
					if ($this->getStore()->getCode() == Mage_Core_Model_Store::ADMIN_CODE) {
						...
						foreach ($allproduct->getWebsiteIds() as $websiteId) {
							...
							foreach ($allstoreids as $storeId) {
								...
								$this->exportProductData($product, $this->getVar('export_multi_store'), $i, $websiteCode);
							}
							$overallcount+=1;
						}
					...
					$this->setProductTypeInstance($product);
					/* @var $product Mage_Catalog_Model_Product */
					$this->exportProductData($product, $this->getVar('export_multi_store'), $i);	
