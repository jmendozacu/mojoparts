...
$connection->update($tableName, array(
    'local_shipping_rate_table' => json_encode($enabledShippingRateTable)
), array(
    'local_shipping_rate_table_mode' => 1
));

$connection->update($tableName, array(
    'international_shipping_rate_table' => json_encode($enabledShippingRateTable)
), array(
    'international_shipping_rate_table_mode' => 1
));

$connection->update($tableName, array(
    'local_shipping_rate_table' => json_encode($disabledShippingRateTable)
), array(
    'local_shipping_rate_table_mode' => 0
));

$connection->update($tableName, array(
    'international_shipping_rate_table' => json_encode($disabledShippingRateTable)
), array(
...
foreach (array_chunk($originalVariationsIds, 1000) as $originalVariationsIdsPart) {
    ...
    $stmt = $connection->query(<<<SQL
        ...
        SET `add` = 0, `delete` = 0
        WHERE `id` IN ({$originalVariationsIdsPart});
SQL
    );
}
