...
class CommerceExtensions_Orderimportexport_Model_Convert_Adapter_Importorders 
...

{
...

    {
        ...
        if(!isset($this->_orderCreateModel)){
            ...
            $this->_orderCreateModel = Mage::getModel("commerceextensions_orderimportexport/convert_adapter_ordercreate");
...
//        }
...

	{
		...
		if($this->getBatchParams('order_map_website_codes') != "") {
			$mapped_store_values = explode(",",$this->getBatchParams('order_map_website_codes'));
			foreach ($mapped_store_values as $mapped_values) {
				$mapped_value = explode("=",$mapped_values);
				if($importData['website'] == $mapped_value[0]) {
					$importData['website'] = $mapped_value[1];
				}
			}
		}
		
		if($this->getBatchParams('order_map_store_ids') != "") {
			$mapped_store_values = explode(",",$this->getBatchParams('order_map_store_ids'));
			foreach ($mapped_store_values as $mapped_values) {
				$mapped_value = explode("=",$mapped_values);
				if($importData['store_id'] == $mapped_value[0]) {
					$importData['store_id'] = $mapped_value[1];
				}
			}
		}
        ...
        foreach ($products_ordered as $data) {
			...
			try {
					...
					$this->handleProduct($data, $add_products_array, $importData, $product, $productcounter, $fullSKU);
			} catch (Exception $e) {
        ...
        try {
            ...
            foreach($order1->getQuote()->getAllItems() as $quoteItem) {
                ...
                if($quoteItem->getProduct()->getSku() == $final_key) {
					...
					$partsforconfigurable = explode('^', $products_ordered[$qouteproductcounter-1]);
					if(isset($partsforconfigurable[2]) && $partsforconfigurable[2] !="") {
						$configurable_parts = explode(':', $partsforconfigurable[0]);
					} else {
						$configurable_parts = explode(':', $products_ordered[$qouteproductcounter-1]);
					}
					$imported_product_type = $configurable_parts[2];
					
					if($imported_product_type == "configurable") { 
...

    {
        ...
        if ($customer->getId()) {
			...
			if ($this->getBatchParams('update_customer_address') == "true") {
				...
				$street_s = array("0"=>$importData['shipping_street_full'], "1"=>isset($importData['shipping_street_2']) ? $importData['shipping_street_2'] : "");
...

    {
        ...
        if (isset($parts[3]) && $parts[2] != "configurable" && $parts[2] != "bundle" && $parts[2] != "simple") {
            ...
            try {
                ...
                $write_qry2 = $write->query("UPDATE `" . $prefix . "sales_flat_order_item` SET name = \"" . addslashes($customproductname) . "\", sku = \"" . $customproductsku . "\" WHERE item_id = '" . $db_item_id . "'");

                $write_qry2 = $write->query("UPDATE `" . $prefix . "sales_flat_quote_item` SET name = \"" . addslashes($customproductname) . "\", sku = \"" . $customproductsku . "\" WHERE quote_id = '" . $db_item_id . "'");
